            ...
            if (e.RowIndex >= 0)
            {
                ...
                else
                {
                    ...
                    if (selectedTimes >= 2)
                    {
                        ...
                        if (selectedOrder != null)
                        {
                            DetailEdit detailEdit = new DetailEdit(selectedOrder.OrderDeatilsList);
                            if (detailEdit.ShowDialog() == DialogResult.OK)
                            {
                                List<OrderDeatils> updatedDetails = detailEdit.GetUpdatedOrderDetailsList();
                                selectedOrder.getOrderDeatilsList().Clear();
                                selectedOrder.getOrderDeatilsList().AddRange(updatedDetails);
                                UpdateOrderList();
                            }
                        }
                    }
